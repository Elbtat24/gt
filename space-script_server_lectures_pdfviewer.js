...
// =====================
// Vote System (تصويت الهدايا) - واتساب ستايل
// =====================

const FIRESTORE_VOTE_POLL_COLLECTION = 'votePoll_v1';
const FIRESTORE_VOTE_POLL_DOC_ID = 'active';
const FIRESTORE_VOTE_SUBCOLLECTION = 'votes';

let __voteInitialized = false;
let __votePollUnsub = null;
let __voteVotesUnsub = null;
let __votePollData = null;
let __voteVotesCache = new Map(); // uid -> {choice, pollVersion}
let __voteBusy = false;

function normalizeVoteText(input, { maxLen = 120 } = {}) {
    ...
    for (const w of bannedWords) {
        ...
        if (re.test(lower) || re.test(s)) return null;
    }

    // يسمح بحروف/أرقام/مسافات وبعض الرموز البسيطة
    const ok = /^[\u0600-\u06FFa-zA-Z0-9 \-()_.,!?+&]+$/;
...
function getVoteUiEls() {
    return {
        q: document.getElementById('voteQuestionText'),
        list: document.getElementById('voteOptionsList'),
        total: document.getElementById('voteTotalVotes'),
        status: document.getElementById('voteStatusText')
    };
}

function voteGetPollRef() {
    const api = window.firestoreApi;
    const db = window.firestoreDb;
    return api.doc(db, FIRESTORE_VOTE_POLL_COLLECTION, FIRESTORE_VOTE_POLL_DOC_ID);
}

function voteGetVotesColRef() {
    const api = window.firestoreApi;
    const db = window.firestoreDb;
    return api.collection(db, FIRESTORE_VOTE_POLL_COLLECTION, FIRESTORE_VOTE_POLL_DOC_ID, FIRESTORE_VOTE_SUBCOLLECTION);
}

function getCurrentUserVoteChoice() {
    const uid = String(appData?.firebaseUser?.uid || '');
    if (!uid) return null;
    const v = __voteVotesCache.get(uid);
    if (!v) return null;
    const pv = Number(__votePollData?.pollVersion || 1);
    if (Number(v.pollVersion || 1) !== pv) return null;
    return typeof v.choice === 'number' ? v.choice : null;
}
...
function computeVoteStats() {
    ...
    __voteVotesCache.forEach(v => {
        if (!v) return;
        if (Number(v.pollVersion || 1) !== pv) return;
...
function renderVotePage() {
    const { q, list, total, status } = getVoteUiEls();
    if (!q || !list || !total) return;

    const poll = __votePollData || {};
    ...
    const hasPoll = question && options.length === 3 && filled.length === 3;

    if (!hasPoll) {
        q.textContent = 'لا يوجد تصويت مُعد حالياً.';
        list.innerHTML = '';
        total.textContent = '—';
        if (status) status.innerHTML = '<i class="fas fa-triangle-exclamation"></i> اطلب من الأدمن يضيف السؤال والاختيارات من صفحة الإعدادات.';
        return;
    }

    q.textContent = question;

    const { counts, total: totalVotes, pcts } = computeVoteStats();
    const myChoice = getCurrentUserVoteChoice();

    total.textContent = `إجمالي الأصوات: ${totalVotes}`;

    const html = options.map((opt, idx) => {
        const pct = pcts[idx] || 0;
        const count = counts[idx] || 0;
        const selected = myChoice === idx;
        const you = selected ? `<span class="vote-option-you"><i class="fas fa-check"></i> اختيارك</span>` : '';
        return `
            <div class="vote-option ${selected ? 'selected' : ''} ${__voteBusy ? 'disabled' : ''}" role="button" tabindex="0"
                 onclick="handleVoteSelect(${idx})" onkeydown="if(event.key==='Enter' || event.key===' '){ event.preventDefault(); handleVoteSelect(${idx}); }">
                <div class="vote-option-header">
                    <span class="vote-option-name">${selected ? '<i class="fas fa-circle-check"></i>' : '<i class="far fa-circle"></i>'} ${escapeHtml(opt)}</span>
                ...
                <div class="vote-option-meta">
                    <span class="vote-option-count">${count} صوت</span>
                    ${you}
                </div>
            </div>
        `;
    ...
    if (status) {
        status.innerHTML = myChoice === null
            ? '<i class="fas fa-hand-pointer"></i> اختار اختيار واحد من فوق.'
            : '<i class="fas fa-rotate"></i> لو عايز تغيّر اختيارك اختار اختيار تاني.';
    }
}

function stopVoteListeners() {
    try { if (__votePollUnsub) __votePollUnsub(); } catch (e) {}
    try { if (__voteVotesUnsub) __voteVotesUnsub(); } catch (e) {}
    __votePollUnsub = null;
    __voteVotesUnsub = null;
}

function initVotePage() {
    if (!isFirestoreReady()) return;
    if (__voteInitialized) {
        renderVotePage();
        return;
    }
    __voteInitialized = true;

    const api = window.firestoreApi;
    const pollRef = voteGetPollRef();

    __votePollUnsub = api.onSnapshot(pollRef, (snap) => {
        if (snap.exists()) {
            __votePollData = snap.data() || {};
        } else {
            __votePollData = null;
        }
        renderVotePage();
    ...
    __voteVotesUnsub = api.onSnapshot(voteGetVotesColRef(), (snap) => {
        __voteVotesCache.clear();
        snap.forEach(docSnap => {
            const d = docSnap.data() || {};
            __voteVotesCache.set(docSnap.id, { choice: d.choice, pollVersion: d.pollVersion });
        });
        renderVotePage();
    });
}

async function handleVoteSelect(choiceIndex) {
    if (__voteBusy) return;
    if (!isFirestoreReady()) return;
    if (!appData?.firebaseUser?.uid) {
        showAlert('لازم تسجّل دخول الأول.', 'error');
        return;
    }
    ...
    if (options.length !== 3) {
        showAlert('التصويت غير جاهز حالياً.', 'error');
        return;
    }
    ...
    const db = window.firestoreDb;
    const uid = String(appData.firebaseUser.uid);
    const pv = Number(poll.pollVersion || 1);

    try {
        const voteRef = api.doc(db, FIRESTORE_VOTE_POLL_COLLECTION, FIRESTORE_VOTE_POLL_DOC_ID, FIRESTORE_VOTE_SUBCOLLECTION, uid);
        await api.setDoc(voteRef, {
            choice: idx,
            pollVersion: pv,
            displayName: getEffectiveUserName() || getSavedFirstName() || undefined,
            updatedAtMs: Date.now(),
            updatedAt: api.serverTimestamp()
        }, { merge: true });
    } catch (e) {
        console.warn('Vote failed', e);
        showAlert('حصل خطأ أثناء التصويت. جرّب تاني.', 'error');
    } finally {
        __voteBusy = false;
        renderVotePage();
    }
...
let __voteAdminBound = false;
let __voteAdminUnsub = null;

function setVoteAdminStatus(text, type = 'info') {
    const el = document.getElementById('voteAdminStatus');
    if (!el) return;
    const icon = type === 'success' ? 'fa-circle-check' : type === 'error' ? 'fa-triangle-exclamation' : 'fa-circle-info';
    el.innerHTML = `<i class="fas ${icon}"></i> ${text}`;
}

function bindVoteAdminOnce() {
    if (__voteAdminBound) return;
    __voteAdminBound = true;

    const q = document.getElementById('voteQuestionInput');
    const o1 = document.getElementById('voteOpt1Input');
    const o2 = document.getElementById('voteOpt2Input');
    const o3 = document.getElementById('voteOpt3Input');

    // Enter = حفظ
    [q, o1, o2, o3].forEach(el => {
        if (!el) return;
        el.addEventListener('keydown', (ev) => {
            if (ev.key === 'Enter') {
                ev.preventDefault();
                saveVotePollFromAdmin();
            }
        }, { passive: false });
    });
}

function initVoteAdminUI() {
    if (!isCurrentUserAdminRole()) return;
    if (!isAdminUnlocked()) return;
    if (!isFirestoreReady()) return;
    ...
    if (__voteAdminUnsub) {
        try { __voteAdminUnsub(); } catch (e) {}
        __voteAdminUnsub = null;
    }

    __voteAdminUnsub = api.onSnapshot(pollRef, (snap) => {
        ...
        if (q && document.activeElement !== q) q.value = String(data?.question || '');
        const opts = Array.isArray(data?.options) ? data.options : [];
        if (o1 && document.activeElement !== o1) o1.value = String(opts[0] || '');
        if (o2 && document.activeElement !== o2) o2.value = String(opts[1] || '');
        if (o3 && document.activeElement !== o3) o3.value = String(opts[2] || '');

        const pv = Number(data?.pollVersion || 1);
        const updatedAtMs = Number(data?.updatedAtMs || 0);
        const when = updatedAtMs ? new Date(updatedAtMs).toLocaleString('ar-EG') : '—';
        setVoteAdminStatus(`نسخة التصويت الحالية: <b>${pv}</b> — آخر تحديث: <b>${when}</b>`, 'info');
    });
}

async function ensureVotePollExistsIfMissing() {
    if (!isFirestoreReady()) return false;
    const api = window.firestoreApi;
    const ref = voteGetPollRef();
    const snap = await api.getDoc(ref);
    if (snap.exists()) return true;

    try {
        await api.setDoc(ref, {
            question: 'عايز هدية إيه للامتحان الجاي؟',
...
async function saveVotePollFromAdmin() {
    if (!isCurrentUserAdminRole()) { showAlert('للأدمن فقط.', 'error'); return; }
    if (!requireAdminForChallengeEdit()) return;
    if (!isFirestoreReady()) return;

    const qEl = document.getElementById('voteQuestionInput');
    const o1El = document.getElementById('voteOpt1Input');
    const o2El = document.getElementById('voteOpt2Input');
    const o3El = document.getElementById('voteOpt3Input');

    const question = normalizeVoteText(qEl?.value, { maxLen: 120 });
    const o1 = normalizeVoteText(o1El?.value, { maxLen: 60 });
    const o2 = normalizeVoteText(o2El?.value, { maxLen: 60 });
    const o3 = normalizeVoteText(o3El?.value, { maxLen: 60 });

    if (!question) { showAlert('اكتب سؤال صحيح (بدون ألفاظ ممنوعة/رموز غريبة).', 'error'); qEl?.focus(); return; }
    if (!o1 || !o2 || !o3) { showAlert('لازم تكتب ٣ اختيارات صحيحة.', 'error'); return; }
    ...
    try {
        let pv = 1;
        const snap = await api.getDoc(ref);
        if (snap.exists()) pv = Number(snap.data()?.pollVersion || 1);
        ...
        showAlert('تم حفظ التصويت ✅', 'success');
        setVoteAdminStatus('تم حفظ السؤال والاختيارات بنجاح ✅', 'success');
    } catch (e) {
        console.warn('saveVotePollFromAdmin failed', e);
        showAlert('حصل خطأ أثناء الحفظ.', 'error');
        setVoteAdminStatus('حصل خطأ أثناء الحفظ.', 'error');
    }
}

async function resetVotePoll() {
    if (!isCurrentUserAdminRole()) { showAlert('للأدمن فقط.', 'error'); return; }
    if (!requireAdminForChallengeEdit()) return;
    if (!isFirestoreReady()) return;
    ...
    try {
        await ensureVotePollExistsIfMissing();
        const snap = await api.getDoc(ref);
        const data = snap.exists() ? (snap.data() || {}) : {};
        const next = Number(data.pollVersion || 1) + 1;
        ...
        showAlert('تم تصفير الأصوات ✅', 'success');
        setVoteAdminStatus(`تم تصفير الأصوات — نسخة جديدة: <b>${next}</b> ✅`, 'success');
    } catch (e) {
        console.warn('resetVotePoll failed', e);
        showAlert('حصل خطأ أثناء تصفير الأصوات.', 'error');
        setVoteAdminStatus('حصل خطأ أثناء تصفير الأصوات.', 'error');
    }
}

function previewVotePageFromAdmin() {
    showSection('vote');
}
