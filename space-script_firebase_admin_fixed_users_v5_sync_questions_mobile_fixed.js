...
function initStars() {
    ...
    const lite = prefersReduced || isCoarse || lowMem || lowCPU;

    if (lite) html.classList.add('lite-mode');

    // Use desynchronized context when available (can reduce jank on some browsers)
    ...
    const onPointerMove = (e) => {
        ...
        pointer.tx = (p.clientX / (w || 1) - 0.5) * 2;
        pointer.ty = (p.clientY / (h || 1) - 0.5) * 2;
    };

    // Buffer: render the heavy gradients once per resize
    ...
    function buildBackdrop() {
        ...
        // Soft haze
        const haze = bctx.createRadialGradient(cx, cy, 0, cx, cy, R);
        haze.addColorStop(0, 'rgba(0,255,255,0.035)');
        haze.addColorStop(0.35, 'rgba(120,70,255,0.028)');
        haze.addColorStop(1, 'rgba(0,0,0,0)');
        bctx.fillStyle = haze;
        bctx.fillRect(0, 0, w, h);

        // Accretion disk (subtle)
        const disk = bctx.createRadialGradient(cx, cy, 30, cx, cy, 160);
        disk.addColorStop(0, 'rgba(0,0,0,0.88)');
        disk.addColorStop(0.45, 'rgba(0,0,0,0.95)');
        disk.addColorStop(0.62, 'rgba(0,255,255,0.06)');
        ...
        bctx.beginPath();
        bctx.arc(cx, cy, 46, 0, Math.PI * 2);
        bctx.fill();
    }

    function buildStars() {
        const area = w * h;
        const base = Math.floor(area / (lite ? 9500 : 7000));
        const count = Math.max(lite ? 120 : 200, Math.min(lite ? 220 : 360, base));
        ...
        for (let i = 0; i < count; i++) {
            ...
            const speed = (lite ? 0.00020 : 0.00035) + Math.random() * (lite ? 0.00025 : 0.00055);
            const size = (Math.random() < 0.88) ? 1 : 2;
            const alpha = 0.10 + Math.random() * 0.30;
            const tw = Math.random() * 2000;
            stars[i] = { r, ang, speed, size, alpha, tw };
        }
    }

    function resize() {
        // Keep DPR=1 for smoothness (massive win on mobiles/retina)
    ...
    // Static render for lite / reduced motion
    function renderStatic() {
        ctx.clearRect(0, 0, w, h);
        ctx.drawImage(buffer, 0, 0);
        // draw a very small amount of stars once
        ctx.fillStyle = 'rgba(255,255,255,1)';
        for (let i = 0; i < stars.length; i += 2) {
            const s = stars[i];
            const x = cxBase() + Math.cos(s.ang) * s.r;
            const y = cyBase() + Math.sin(s.ang) * s.r * 0.82;
    ...
    function tick(ts) {
        ...
        const cx = cxBase() + pointer.x * 14;
        const cy = cyBase() + pointer.y * 9;
        const tilt = 0.82;
        ...
        for (let i = 0; i < stars.length; i++) {
            ...
            // very subtle inward drift, loop back
            s.r -= 0.015 * dt;
            if (s.r < 70) s.r = Math.hypot(w, h) * 0.62;

            const x = cx + Math.cos(s.ang) * s.r;
            const y = cy + Math.sin(s.ang) * s.r * tilt;

            const tw = 0.65 + 0.35 * Math.sin((ts + s.tw) * 0.004 + i);
...
function initCinematicUI() {
    ...
    const dotsHost = document.getElementById('homeSlideDots');
    if (!homeSlides || !dotsHost) return;

    const slides = Array.from(homeSlides.querySelectorAll('.hesty-slide'));
    ...
    const dots = slides.map((slide, idx) => {
        ...
        dot.setAttribute('aria-label', `انتقال للشريحة رقم ${idx + 1}`);
        dot.addEventListener('click', () => {
            homeSlides.scrollTo({ top: slide.offsetTop, behavior: 'smooth' });
        });
        dotsHost.appendChild(dot);
        return dot;
    });

    // reveal animations inside scroll container
    const revealTargets = [];
    slides.forEach(slide => {
        const direct = Array.from(slide.children);
        direct.forEach(el => {
    ...
    const observer = new IntersectionObserver(
        ...
        { root: homeSlides, threshold: 0.12 }
    );
    ...
    const update = () => {
        ticking = false;
        const y = homeSlides.scrollTop + homeSlides.clientHeight * 0.4;
        let activeIdx = 0;
        for (let i = 0; i < slides.length; i++) {
            const sTop = slides[i].offsetTop;
            const sBottom = sTop + slides[i].offsetHeight;
            if (y >= sTop && y < sBottom) { activeIdx = i; break; }
        }
        dots.forEach((d, i) => d.classList.toggle('active', i === activeIdx));
    };

    homeSlides.addEventListener('scroll', () => {
        if (!ticking) {
            ticking = true;
